...
int OAHT_Hash2(KeyType Key, int KeyLength, int TableSize)
{
    ...
    // 나머지 연산으로 인해 같은 값을 갖지 않도록 
    // 테이블의 크기에서 3을 뺀 수로 나눈다
...
void OAHT_Rehash(HashTable** HT)
{
    ...
    printf("\nRehashed. New table size is : %n\n\n", NewHT->TableSize);

    // 이전의 해시 테이블에 이썯ㄴ 데이터를 새 해시 테이블로 옮긴다.
...
void OAHT_Set(HashTable** HT, KeyType Key, ValueType Value)
{
    ...
    if (Usage > 0.5) // 점유율이 50%를 넘으면 재해싱을 수행한다. (50%는 작성자 맘입니다.)
    {
        OAHT_Rehash(HT);
    }
